...
Use `async-openai` 0.41 (features `chat-completion`, `chat-completion-types`, **`rustls`**) instead of raw reqwest. The `rustls` feature is required: async-openai's default features are disabled to slim the dep tree, and without `rustls` reqwest has no TLS backend — every https request fails with hyper's "invalid URL, scheme is not http" from the plain HTTP connector. Verified: `OpenAIConfig::new().with_api_base(url).with_api_key(key)` covers BYOK; `create_stream` handles SSE; tool calling (including streamed tool-call deltas) is supported. Avoids hand-rolling SSE parsing and request/response types. Caveat: OpenAI-shaped; a small adapter may be needed for exotic providers — acceptable since providers are OpenAI-compatible. Provider base URLs are normalized (a trailing `/chat/completions` suffix is stripped, since providers often document the full endpoint and async-openai appends the path itself).

### D2: Key in the keychain, URL/model in settings
`save_provider` stores the API key in the OS keychain via the `keyring` crate (service `com.foldquery.app`, user `api_key`); URL and model go into the frontend settings store (they are not secrets). The key crosses into Rust exactly once (at save) and is never read back into the webview; `get_provider_status` returns only `{configured, url, model}`. **Gotcha:** keyring requires its native feature per platform (`apple-native` on macOS, `windows-native` on Windows, `sync-secret-service` on Linux) — without it, macOS falls back to the non-persistent mock store, where writes succeed but reads from a different `Entry` return NoEntry and nothing ever reaches the real Keychain. A cross-entry persistence test guards against this.
...
The agent uses a **raw reqwest + eventsource-stream SSE parser** (not async-openai's strict types) so provider quirks don't break the stream — opencode-go emits custom metadata chunks (e.g. `x-opencode-type: inference-cost` with empty `choices` and no `id`) that fail async-openai's deserializer; the parser skips non-standard chunks (empty-choices, non-JSON, `[DONE]`) and only extracts content deltas and tool-call deltas. Requests use `stream: true`; each SSE delta is forwarded to the frontend as `agent-token`. If streaming fails, the request retries once without streaming (`stream: false`, parsed with the same tolerant approach). async-openai remains only for the provider connection test, which uses a plain non-streaming completion.
